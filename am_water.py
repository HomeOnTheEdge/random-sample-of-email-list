...
def import_am_water_HIST(filename=filename):             #  https://stackoverflow.com/a/36416258
    ...
    # Adding voter frequencies:
    am_water_HIST['Freq'] = am_water_HIST.groupby('Voter_ID')['Voter_ID'].transform('size')
...
def explore(df):
    ...
    print(df['Freq'].value_counts(sort=False))
...
def output_supervoter_ids(df):
    supervoters = df.sort_values(by=['Voter_ID'])
    supervoters = supervoters.drop_duplicates(subset=['Voter_ID'], keep='last')
    return supervoters